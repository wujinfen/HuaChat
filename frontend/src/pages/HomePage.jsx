...
import { UsersIcon, CheckCircleIcon, UserPlusIcon, MapPinIcon } from "lucide-react"
...
const HomePage = () => {
  ...
  const { data:friends=[], isLoading:loadingFriends } = useQuery({
    queryKey: ["friends"],
    queryFn: async () => {
      const response = await axiosService.get("/users/friends")
  ...
  const { data: outgoingFriendReqs } = useQuery({
    ...
    queryFn: async () => {
      const response = await axiosService.get("/users/outgoing-friend-requests")
      return response.data
    }
  })
  ...
  const { mutate: sendRequestMutation, isPending } = useMutation({
    mutationFn: async (userId) => {
      const response = await axiosService.post(`/users/friend-request/${userId}`)
      return response.data
    },
    onSuccess: () => {
        ...
        <section>
          ...
          {loadingRecommendedUsers ? (
            ...
            <div className="card bg-base-200 p-6 text-center">
              <h3 className="font-semibold text-lg mb-2">No discoveries </h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {recommendedUsers.map((user) => {
                let hasRequestBeenSent = outgoingRequestsIds.has(user._id);
